...
export default class AsciiTextParser {
    ...
    parse(grid: Grid): Array<Shape> {
        ...
        // create the list of all edges which have been used to create boxes
        const boxEdges = boxes.flatMap(box => [box.topEdge, box.bottomEdge, box.leftEdge, box.rightEdge]);

        const edgesNoBoxes = uniqueEdges.filter(edge => !this.containsEdge(boxEdges, edge));
